...
use std::{fs, path, error, collections, f64, time};
...
impl GenResult {
    fn from_meta_path(meta: &fs::Metadata, path: path::PathBuf) -> GenResult {
        if meta.is_dir() {
            fs::read_dir(&path)
                .map_or_else(|err| GenResult::Single(Some(Err(err.into()))), |rd| GenResult::Dir(rd, None))
            ...
            GenResult::Dir(reader, container) => {
                if let Some(result) = container.as_deref_mut() {
                    ...
                    reader.next().map(|res| {
                        ...
                        *container = Some(Box::new(GenResult::from_meta_path(&meta, entry.path())));
                        Ok(())
...
fn main() -> Result<(), Box<dyn Send + Sync + error::Error>> {
    ...
    let completion = args.images.par_iter()
        .flat_map(|path| {
            let base = path::PathBuf::from(path);
            let res: GenResult = base.clone().into();
            let is_dir = if let GenResult::Dir(_, _) = res { true } else { false }; 
            res.map(move |p| p.and_then(|rp| {
                let rel = if is_dir {
                    rp.strip_prefix(&base)?.to_owned()
                } else {
                    rp.file_name().ok_or::<Box<dyn Send + Sync + error::Error>>("Failed to extract file name".into())?.into()
                };
                Ok((rp, rel))
        })).par_bridge()
        })
        .map(|prev| {
            ...
            for (x, y, &px) in img.enumerate_pixels() {
                ...
                while let Some((x, y)) = q.pop_front() {
                    ...
                    for (dx, dy) in [(0, 1), (0, -1), (1, 0), (-1, 0)] {
                        ...
                        if cx >= img.width() || cy >= img.height() || vis[(cy * img.width() + cx) as usize] || img[(cx, cy)].0[0] > MAX_GRAY {
                            continue;
                        }
                        q.push_back((cx, cy));
                    }
            ...
            for blob in blobs {
                ...
                if blob.iter().copied().any(|(x, y)| (x as f64 - cx).hypot(y as f64 - cy) > allowed_radius) {
                    continue
                }
                for pt in blob {
                    out[pt] = [255].into();
        ...
        .inspect(|res| if let Ok((realpath, target)) = res {
    ...
    println!("Processed {} images in {:.3}s ({} errors)", completion.iter().filter(|v| v.is_ok()).count(), delta_t.as_secs_f64(), completion.iter().filter(|v| v.is_err()).count());
    Ok(())
}
